...
namespace API.Endpoints.Accounts.Services
{
    ...
    /// </summary>
    public class Friends : Gale.REST.Http.HttpReadActionResult<String>
    {
        ...
        public Friends(String user, String query, int offset, int limit) : base(user) {
            _query = query;
            _offset = offset;
            _limit = limit;
        }
        ...
        {
            using (Gale.Db.DataService svc = new Gale.Db.DataService("SP_GET_AccountFriends"))
            {
                ...
                List<Models.Friend> friends = repo.GetModel<Models.Friend>(1);

                pagination.items = friends;
